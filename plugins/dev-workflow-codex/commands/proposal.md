...
#### 3.1 Delegate to code-explorer

When the change is non-trivial, delegate focused codebase analysis to the `code-explorer` agent with:
...
#### 3.2 Delegate to code-architect

After analysis is available, delegate the implementation blueprint to the `code-architect` agent with:
...
#### 3.4 Fallback (if delegation is unnecessary or unavailable)
